...
# Validate wrapper jar existence
if [ ! -f "$CLASSPATH" ]; then
    if command -v gradle >/dev/null 2>&1; then
        exec gradle "$@"
    else
        die "ERROR: Gradle wrapper JAR not found at $CLASSPATH. Please ensure the 'gradle/wrapper' directory exists and contains 'gradle-wrapper.jar'."
    fi
fi
